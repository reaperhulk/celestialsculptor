...
export function auditIterations(text, minimum = 100) {
  ...
  assert.ok(
    ...
    `Expected at least ${minimum} completed iterations, found ${entries.length}`,
  );
  entries.forEach((entry, index) => {
    ...
    for (const field of ['Review needs', 'Implemented', 'Validation'])
      assert.match(
        section,
        new RegExp(`^${field}:\\s*\\S`, 'm'),
        `Iteration ${entry[1]} lacks ${field}`,
      );
  });
