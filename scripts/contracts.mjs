...
export async function verifyToolchainContracts() {
  const [cargo, wasm, toolchain, workflow, qualification, packageText, lockText] =
    await Promise.all(
      [
        'Cargo.toml',
  ...
  for (const text of [workflow, qualification]) {
    for (const [, pin] of text.matchAll(/toolchain: ([^\n]+)/g))
