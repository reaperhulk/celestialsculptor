...
assert.equal(
  execFileSync('wasm-bindgen', ['--version'], { encoding: 'utf8' }).trim(),
  `wasm-bindgen ${versions.bindgen}`,
  'Install the binding generator version pinned in crates/wasm/Cargo.toml',
);
...
for (const name of await readdir('dist'))
  if (minified(name))
    await writeFile(
      `dist/${name}`,
      await minifySource(name, await readFile(`dist/${name}`, 'utf8')),
    );
// Keep symbol-bearing output for profiling while shipping only executable data.
run('wasm-bindgen', [
  'target/wasm32-unknown-unknown/release/celestial_wasm.wasm',
  '--target',
  'web',
  '--out-dir',
  'target/browser-symbols',
  '--out-name',
  'celestial_wasm',
]);
run('wasm-bindgen', [
...
await writeFile(
  ...
  JSON.stringify({
    ...
    assets: await assetManifest('dist'),
  }),
);
