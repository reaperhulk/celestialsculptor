...
const trajectory = raw.runs[0].samples.map((sample, index) => {
  ...
  let p = 0;
  for (let i = 5; i < sample.state.length; i += 5)
    p +=
      (sample.state[i] - reference.state[i]) ** 2 +
      (sample.state[i + 1] - reference.state[i + 1]) ** 2;
  return { year: sample.year, rmsPositionAu: Math.sqrt(p / (raw.bodies - 1)) };
});
...
if (trajectory.some((s) => s.rmsPositionAu > 1e-3))
  failures.push('Tree/direct trajectory RMS exceeds 0.001 AU');
const report = {
  schema: 1,
  physics: raw.physics,
  bodies: raw.bodies,
  ...
  scope:
    'Constant 512-body collisionless cold disk; full 600-year tree lifetime, independently summed direct-force comparison at the same timestep. Low debris mass isolates force accumulation from hard encounters.',
  rows,
  trajectory,
