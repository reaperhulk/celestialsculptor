...
for (const fixture of native.cases) {
  ...
  const probe = new Simulation(JSON.stringify(fixture.replay.config));
  importReplay(probe, JSON.stringify(fixture.replay));
  const snapshotBytes = Buffer.byteLength(probe.snapshot());
  const started = performance.now();
  for (let i = 0; i < 200; i++) JSON.parse(probe.snapshot());
  const snapshotMicroseconds = (performance.now() - started) * 5;
  probe.free();
  samples.sort((a, b) => a - b);
  assert.ok(samples.every((sample) => Number.isFinite(sample) && sample > 0));
  cases.push({
    snapshotBytes,
    snapshotMicroseconds,
    bodies: fixture.bodies,
    nativeMedianMs: fixture.median_ms,
    wasmMedianMs: samples[3],
    wasmMaxMs: samples[6],
    wasmTicksPerSecond: 2048000 / samples[3],
  });
}
  ...
  await appendFile(
    ...
    '## Simulation performance\n\nInformational medians; correctness gates use numerical invariants, not runner speed.\n\n| Bodies | Native ms / 2048 ticks | WASM ms / 2048 ticks | Snapshot µs | Snapshot bytes |\n|---:|---:|---:|---:|---:|\n' +
      cases
        .map(
          (c) =>
            `| ${c.bodies} | ${c.nativeMedianMs.toFixed(1)} | ${c.wasmMedianMs.toFixed(1)} | ${c.snapshotMicroseconds.toFixed(1)} | ${c.snapshotBytes} |`,
        )
        .join('\n') +
      '\n',
  );
