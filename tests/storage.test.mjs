...
test('autosave preserves a previous experiment and recovers corrupt primary', () => {
  const s = memory(),
    next = replay.replace('"end_tick":0', '"end_tick":8');
  assert.ok(saveExperiment(s, replay));
  assert.ok(saveExperiment(s, next));
  assert.equal(s.getItem(BACKUP_KEY), replay);
  s.setItem(SAVE_KEY, 'broken');
  assert.deepEqual(savedExperiments(s), [replay]);
});
test('invalid and oversized imports fail before simulation work', () => {
  for (const text of [
    ...
    '{}',
    'x'.repeat(512001),
    replay.replace('"end_tick":0', '"end_tick":307201'),
  ])
    assert.throws(() => parseReplay(text));
  assert.equal(saveExperiment(null, replay), false);
  assert.deepEqual(savedExperiments(null), []);
});
test('portable backup includes normalized progress and preserves raw replay compatibility', () => {
  ...
  assert.deepEqual(archive.profile.completed, [0, 1]);
  assert.deepEqual(JSON.parse(archive.replay), JSON.parse(replay));
...
test('saving after recovery cannot replace the good backup with corrupt data', () => {
  ...
  assert.ok(saveExperiment(quota, next));
  assert.equal(s.getItem(SAVE_KEY), next);
});
test('an untrusted primary with valid JSON cannot replace a known recovery snapshot', () => {
  ...
  s.setItem(
    ...
    JSON.stringify({
      version: 7,
      config: { seed: -1, mission: 99, star_mass: 0 },
      commands: [],
      end_tick: 0,
    }),
  );
...
test('only the current simulation format is accepted by browser storage', () => {
  for (const version of [1, 2, 3, 4, 5, 6, 8])
    assert.throws(() => parseReplay(JSON.stringify({ ...JSON.parse(replay), version })));
