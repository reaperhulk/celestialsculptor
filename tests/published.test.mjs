...
function fetcher({stale=false,badMime=false,missing=false,badRoot=false}={}){return async url=>{
 const relative=url.pathname.replace('/celestialsculptor/','');if(badRoot&&!relative)return new Response('Old root page');const path=relative||'index.html';
...
test('published verification checks every exact asset and the WASM serving type',async()=>{
 ...
 await assert.rejects(verifyPublished('https://example.test/celestialsculptor/',info.revision,fetcher({badRoot:true})),/Root entrypoint/);
});
