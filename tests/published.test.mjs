...
function fetcher({ stale = false, badMime = false, missing = false, badRoot = false } = {}) {
  return async (url) => {
    const relative = url.pathname.replace('/celestialsculptor/', '');
    if (badRoot && !relative) return new Response('Old root page');
    ...
    return new Response(bytes, {
      headers: {
        'content-type': path.endsWith('.wasm') && !badMime ? 'application/wasm' : 'text/plain',
      },
    });
  };
}
test('published verification checks every exact asset and the WASM serving type', async () => {
  const result = await verifyPublished(
    'https://example.test/celestialsculptor/',
    info.revision,
    fetcher(),
  );
  assert.equal(result.assets, Object.keys(info.assets).length);
  await assert.rejects(
    verifyPublished('https://example.test/celestialsculptor/', 'stale', fetcher()),
    /deployed revision/,
  );
  await assert.rejects(
    verifyPublished(
