...
test('swarm settings preserve resolution only for the current replay rules',()=>{
 const value={version:7,config,command:{type:'generate_system',style:'swarm',count:8191,chaos:0}};const read=v=>readGeneration({getItem:()=>JSON.stringify(v)});assert.deepEqual(read(value),value);assert.equal(read({...value,version:6}),null);assert.equal(read({...value,version:5}),null);assert.equal(read({...value,command:{...value.command,count:8192}}),null);
});
test('published build metadata agrees with the actual replay rules',async()=>{
 const build=JSON.parse(await readFile('dist/build-info.json','utf8')),s=new Simulation(JSON.stringify(config));try{assert.equal(build.saveVersion,JSON.parse(s.export_replay()).version);}finally{s.free();}
});
