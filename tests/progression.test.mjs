...
import {
  ...
  readProfile,
  writeProfile,
} from '../web/progression.js';
test('all ten missions unlock in order with idempotent awards', () => {
  let p = normalizeProfile(null);
  for (let i = 0; i < 10; i++) {
    assert.equal(nextMission(p), i);
    assert.ok(canPlay(p, i));
    if (i < 9) assert.equal(canPlay(p, i + 1), false);
    p = award(p, i);
    assert.deepEqual(award(p, i), p);
  }
  assert.equal(p.completed.length, 10);
  assert.equal(canPlay(p, 10), false);
});
test('corrupt and unavailable storage do not prevent play', () => {
  ...
  assert.deepEqual(normalizeProfile({ version: 1, completed: [9, 9, -1, '0', null] }), {
    version: 1,
    completed: [],
  });
