...
test('the full seeded outcome matrix reproduces native body identities and trajectories in WASM', () => {
  ...
  for (const example of report.cases) {
    const sim = new Simulation(JSON.stringify(example.replay.config));
    try {
      importReplay(sim, JSON.stringify(example.replay));
      const actual = JSON.parse(sim.snapshot());
      assert.equal(actual.bodies.length, example.bodies);
      for (const [index, a] of actual.bodies.entries()) {
        const b = example.final_bodies[index];
        ...
        for (const field of ['pos', 'vel'])
          for (const axis of ['x', 'y'])
            assert.ok(
              Math.abs(a[field][axis] - b[field][axis]) < 1e-8,
              `${example.style}/${example.seed}/${field}.${axis}`,
            );
      }
    } finally {
      sim.free();
