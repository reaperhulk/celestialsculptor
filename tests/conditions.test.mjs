...
test('launch fields reject missing and nonfinite values instead of previewing a stellar impact', () => {
  ...
  assert.deepEqual(parseLaunchFields(fields), {
    kind: 'rocky',
    radius: 1,
    speed: 1,
    angle: Math.PI / 2,
  ...
  for (const [key, value] of [
    ...
    assert.throws(() => parseLaunchFields({ ...fields, [key]: value }));
});
...
test('placement guidance follows Rust availability and explains exhausted resources', () => {
  const status = {
    available_slots: 4,
    actions_remaining: 9,
    tools: [{ kind: 'rocky', cost: 1, unlocked: true, affordable: true }],
  };
  assert.equal(placementIssue(status, 'rocky'), '');
  assert.match(placementIssue(status, 'giant'), /unlocks/);
  ...
  assert.match(
    placementIssue({ ...status, tools: [{ ...status.tools[0], affordable: false }] }, 'rocky'),
    /1 matter/,
  );
});
test('custom masses use their actual cost and range instead of the kind default', () => {
  const status = {
    ...
    actions_remaining: 7,
    tools: [
      { kind: 'rocky', cost: 1, min_mass: 0.1, max_mass: 10, unlocked: true, affordable: false },
    ],
  };
  assert.equal(placementIssue(status, 'rocky', 0.1), '');
  assert.match(placementIssue(status, 'rocky', 1), /1 matter/);
  assert.match(placementIssue(status, 'rocky', 0.05), /range/);
  assert.match(placementIssue(status, 'rocky'), /1 matter/);
});
