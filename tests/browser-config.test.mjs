...
test('Chromium launch settings cannot leak through global defaults into other engines', () => {
  ...
  for (const project of config.projects) {
    engines.add(project.use.browserName);
    if (project.use.browserName === 'chromium') assert.equal(project.use.channel, 'chromium');
    else {
      assert.equal(project.use.channel, undefined);
      assert.equal(project.use.launchOptions, undefined);
    }
  }
