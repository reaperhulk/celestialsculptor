...
class Context {
  ...
  async resume() {
    if (this.reject) throw Error('gesture required');
    this.state = 'running';
  }
  async suspend() {
    this.state = 'suspended';
...
test('audio only reports enabled after successful resume and bounds voice resources', async () => {
  ...
  try {
    ...
    await sound.toggle();
    assert.equal(sound.master.gain.value, 0.03);
    sound.setVolume(2);
    assert.equal(sound.volume, 1);
    sound.setVolume(NaN);
    assert.equal(sound.volume, 1);
    assert.equal(sound.enabled, true);
    for (let i = 0; i < 50; i++) sound.event('collision');
    assert.equal(sound.voices, 12);
    const notes = sound.context.nodes.filter((node) => node.onended);
    ...
    await sound.toggle();
    sound.context.reject = true;
    await assert.rejects(sound.toggle(), /gesture/);
    assert.equal(sound.enabled, false);
    sound.context.reject = false;
    await sound.toggle();
    await sound.visibility(true);
    assert.equal(sound.context.state, 'suspended');
    await sound.visibility(false);
