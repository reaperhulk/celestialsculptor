...
test('a thousand-body swarm renders, advances, pauses and remains navigable',async({page},testInfo)=>{
 ...
 await page.locator('#play').click();await expect(page.locator('#play')).toHaveText('▶ Run');const tick=await page.locator('#universe').getAttribute('data-tick');await page.waitForTimeout(300);expect(await page.locator('#universe').getAttribute('data-tick')).toBe(tick);
 const canvas=await page.locator('#universe').boundingBox();await page.mouse.move(canvas.x+canvas.width*.5,canvas.y+canvas.height*.5);await page.mouse.wheel(0,-300);await expect(page.locator('#fps-overlay')).toContainText('1024 bodies');
