...
test('custom masses, retrograde moons and independent axial spin work through the real worker',async({page})=>{
 ...
 await page.locator('#inspect-body').selectOption('1');await expect(page.locator('#inspector')).toContainText('318 Earth masses');await page.locator('#moon-tools > summary').click();await page.locator('#moon-direction').selectOption('-1');await page.locator('#add-moon').click();await expect(page.locator('#inspect-body option')).toHaveCount(3);
