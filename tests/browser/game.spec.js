...
test('real worker launches a world, completes a goal, and unlocks the next', async ({ page }) => {
  ...
  await expect(page.locator('#mission-name')).toHaveText('A place for life');
...
test('autosave reload restores a paused experiment', async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          JSON.parse(localStorage.getItem('celestial-sculptor.experiment.v1') || '{"commands":[]}')
            .commands.length,
      ),
    )
    .toBe(1);
...
test('an inspected world can be nudged and the edit undone', async ({ page }) => {
  ...
  await expect
    .poll(async () =>
      Number((await page.locator('#inspector').innerText()).match(/e = ([^\n]+)/)[1]),
    )
    .toBeCloseTo(0.21, 3);
...
test('sandbox starting points create editable paused systems', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'A quiet garden' }).click();
  await expect(page.locator('#planet-count')).toHaveText('3');
  await expect(page.locator('#play')).toHaveText('▶ Run');
  await expect(page.locator('#mission-name')).toHaveText('Your universe');
});
test('graphics context loss leaves physics usable and recovery resumes drawing', async ({
  page,
}) => {
  await page.locator('#launch').click();
  await page.evaluate(() => {
    const gl = document.querySelector('canvas').getContext('webgl2');
    window.restoreGraphics = gl.getExtension('WEBGL_lose_context');
    if (!window.restoreGraphics) throw Error('Context-loss test extension missing');
    window.restoreGraphics.loseContext();
  });
  await expect(page.locator('#toast')).toContainText('Graphics paused');
  await page.locator('#step').click();
  await expect(page.locator('#sim-years')).toHaveText('0.03');
  await expect(page.locator('#planet-count')).toHaveText('1');
  await page.evaluate(() => window.restoreGraphics.restoreContext());
  await expect(page.locator('#toast')).toContainText('Graphics restored');
  await expect
    .poll(() =>
      page.evaluate(() => document.querySelector('canvas').getContext('webgl2').isContextLost()),
    )
    .toBe(false);
  await page.locator('#step').click();
  await expect(page.locator('#sim-years')).toHaveText('0.06');
});
test('replacement confirmation pauses time and cancellation resumes the same run', async ({
  page,
}) => {
  await page.locator('#launch').click();
  await page.locator('#play').click();
...
test('rejected stellar conditions keep the existing mode and restore the controls', async ({
  page,
}) => {
  await page.locator('#sandbox').click();
  await page.evaluate(() =>
    document.querySelector('#star-mass').add(new Option('Invalid test mass', '99')),
  );
  await page.locator('#star-mass').selectOption('99');
  await expect(page.locator('#toast')).toContainText('Invalid stellar mass');
  await expect(page.locator('#star-mass')).toHaveValue('1');
  await expect(page.locator('#mission-name')).toHaveText('Your universe');
  await page.locator('#launch').click();
  await expect(page.locator('#planet-count')).toHaveText('1');
});
...
test('scene keyboard controls work without repeating placements or stealing field input', async ({
  ...
  await expect(page.locator('#radius')).toHaveValue('1.05');
  await page.keyboard.press('ArrowLeft');
  await expect(page.locator('#angle')).toHaveValue('5');
  ...
  await page
    ...
    .evaluate((canvas) =>
      canvas.dispatchEvent(
        new KeyboardEvent('keydown', { key: 'l', code: 'KeyL', repeat: true, bubbles: true }),
      ),
    );
  await expect(page.locator('#planet-count')).toHaveText('1');
  await page.keyboard.press('Space');
