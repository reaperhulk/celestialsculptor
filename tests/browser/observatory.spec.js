...
test('inspect an encounter, preserve its original, branch and compare at equal ages',async({page})=>{
 ...
 for(const check of await page.locator('.notebook-entry input[type=checkbox]').all())await check.check();await expect(page.locator('#comparison-age')).toContainText('Both at year');await expect(page.locator('#comparison tbody tr')).toHaveCount(9);
