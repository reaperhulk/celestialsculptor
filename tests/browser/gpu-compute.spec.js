...
test('tiled WebGPU forces agree with the independent double-precision oracle',async({page},testInfo)=>{
 ...
 const report=await page.evaluate(async({fallback})=>{const {runGpuBenchmark}=await import('./gpu-benchmark.js');return runGpuBenchmark({counts:[63,65,257,1024],rounds:3,fallback});},{fallback:software});
 ...
 expect(report.supported,'The configured WebGPU adapter must execute the shader').toBe(true);expect(report.cases).toHaveLength(5);
 for(const result of report.cases){expect(result.rmsRelativeError).toBeLessThan(.001);expect(result.p99RelativeError).toBeLessThan(.005);expect(result.gpuRoundTripMs).toBeGreaterThan(0);expect(result.samples.gpu).toHaveLength(3);}
