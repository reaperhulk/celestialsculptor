...
test('unchanged launch conditions reuse preview geometry across status updates', () => {
  const cache = new PreviewCache(),
    draft = { radius: 1, angle: 0, speed: 1 },
    path = cache.update(draft);
  assert.ok(path.length > 10);
  for (let i = 0; i < 1000; i++)
    assert.equal(cache.update({ ...draft, kind: i % 2 ? 'ice' : 'rocky' }), path);
  assert.notEqual(cache.update({ ...draft, radius: 2 }), path);
  assert.deepEqual(cache.update(null), []);
...
test('launch sites outside the usable canvas are revealed without disturbing visible sites', async () => {
  ...
  for (const p of [
    ...
    assert.equal(draftOutsideView(p, 390, 360), true);
});

test('revealed launch sites fit narrow desktop and phone canvases at every angle', async () => {
  const { draftZoom, draftOutsideView } = await import('../web/preview.js'),
    { project } = await import('../web/geometry.js');
  for (const [width, height] of [
    ...
    for (const tilt of [0.62, 1])
      for (const angle of [0, 0.5, 1, 2, 3, 4, 5, 6]) {
        const d = { radius: 6, angle },
          zoom = draftZoom(d, width, height, tilt, 0.05);
        assert.equal(
          draftOutsideView(
            project(6 * Math.cos(angle), 6 * Math.sin(angle), width, height, zoom, tilt),
            width,
            height,
          ),
          false,
