...
export function installInput(canvas,renderer,{onDraft,onSelect}){
  ...
  canvas.addEventListener('pointerdown',event=>{
    ...
    if(pointers.size===2){const [a,b]=[...pointers.values()];pinch={distance:Math.hypot(a[0]-b[0],a[1]-b[1]),zoom:renderer.zoom};dragging=false;return;}
  ...
  canvas.addEventListener('wheel',event=>{event.preventDefault();renderer.zoom=Math.max(1,Math.min(9,renderer.zoom*Math.exp(Math.max(-150,Math.min(150,event.deltaY))*.002)));},{passive:false});
}
