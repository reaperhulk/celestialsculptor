...
export function wheelZoom(delta, mode = 0, height = 800) {
  ...
  return Math.exp(Math.max(-150, Math.min(150, pixels)) * 0.002);
}
export function launchFromPoint(x, y, star = { x: 0, y: 0 }) {
  const dx = x - star.x,
    dy = y - star.y;
  return {
    radius: Math.max(0.25, Math.min(6, Math.hypot(dx, dy))),
...
export function nearestBody(
  ...
  for (const body of bodies) {
    const p = position(body),
      ...
      drawn = size(body),
      radius = Math.max(threshold, Number.isFinite(drawn) ? drawn : 0);
...
export function installInput(canvas, renderer, { onDraft, onSelect }) {
  ...
  const active = () => {
    renderer.cameraActiveUntil = performance.now() + 800;
  ...
  canvas.addEventListener('pointermove', (event) => {
    ...
    if (pinch && pointers.size === 2) {
      ...
      if (distance > 5) {
        ...
        renderer.zoom = next.zoom;
        renderer.center = next.center;
      }
      return;
    }
    if (!gesture) return;
    ...
    if (gesture.moved && !gesture.started) {
      gesture.started = true;
      gesture.center = { ...renderer.center };
      active();
    }
    if (gesture.moved) {
      if (renderer.inputMode === 'place') updateDraft(event);
      else {
        const now = performance.now(),
          dt = Math.max(8, now - gesture.time),
          before = renderer.center;
        renderer.center = panCenter(
          gesture.center,
          dx,
          dy,
          canvas.clientHeight,
        ...
        gesture.velocity = {
          x: (renderer.center.x - before.x) / dt,
          y: (renderer.center.y - before.y) / dt,
        };
        gesture.time = now;
      }
    }
    ...
    if (gesture && pointers.size === 1) {
      if (!gesture.moved) {
        ...
        if (id !== null) {
          ...
          onSelect();
          if (lastTap?.id === id && performance.now() - lastTap.time < 360) renderer.focus(id);
          lastTap = { id, time: performance.now() };
        } else if (renderer.inputMode === 'place') updateDraft(event);
      } else if (renderer.inputMode !== 'place' && performance.now() - gesture.time < 80) {
        renderer.panVelocity = gesture.velocity;
        renderer.lastCameraTime = performance.now();
      }
    }
  ...
  canvas.addEventListener('dblclick', (event) => {
    ...
    if (renderer.selected !== null) renderer.focus(renderer.selected);
  });
  canvas.addEventListener(
    ...
    (event) => {
      ...
      const next = anchoredZoom(
        ...
        renderer.zoom,
        renderer.tilt,
        renderer.zoom * wheelZoom(event.deltaY, event.deltaMode, canvas.clientHeight),
      );
      renderer.zoom = next.zoom;
      renderer.center = next.center;
    },
