...
export class PreviewCache {
  ...
  update(draft) {
    ...
    if (key !== this.key) {
      this.key = key;
      this.path = draft ? launchPath(draft.radius, draft.angle, draft.speed) : [];
    }
    return this.path;
  }
}
...
export function draftZoom(draft, width, height, tilt, zoom, margin = 32) {
  return (
    Math.max(
      zoom,
      (Math.abs(draft.radius * Math.cos(draft.angle)) * height) / Math.max(1, width - 2 * margin),
      (Math.abs(draft.radius * Math.sin(draft.angle)) * tilt * height) /
