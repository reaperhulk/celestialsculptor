...
export function distance(value) {
  return value < 0.1 ? `${quantity(value * AU_KM)} km` : `${quantity(value)} AU`;
}
export function period(years) {
  return years === null
    ? 'no return'
    : years < 1
      ? `${quantity(years * 365.25)} days`
      : `${quantity(years)} years`;
}
export function orbitReading(body, orbit, host = null) {
  const direction = host
    ? (body.pos.x - host.pos.x) * (body.vel.y - host.vel.y) -
        (body.pos.y - host.pos.y) * (body.vel.x - host.vel.x) <
      0
      ? 'Clockwise'
      : 'Counterclockwise'
    : '';
...
export function spinReading(body, host) {
  const rate = body.spin / (0.4 * body.mass * body.radius * body.radius);
  if (!Number.isFinite(rate)) return 'Axial rotation unavailable.';
  if (rate === 0) return 'No axial rotation.';
  const angular = host
    ? (body.pos.x - host.pos.x) * (body.vel.y - host.vel.y) -
      (body.pos.y - host.pos.y) * (body.vel.x - host.vel.x)
    : 0;
...
export function spinRate(turns, direction) {
  const value = Number(turns);
  if (String(turns).trim() === '' || !Number.isFinite(value) || value < 0 || value > 1000)
    throw new Error('Choose 0–1000 axial turns per year.');
  return value * 2 * Math.PI * direction;
}
