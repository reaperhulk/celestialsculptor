...
export class RequestChannel {
  constructor(post, timeoutMs = 15000) {
    this.post = post;
  ...
  send(type, data = {}) {
    ...
    return new Promise((resolve, reject) => {
      const expire = () => {
        ...
        reject(new Error('The simulation did not respond. Reload to try again.'));
      };
      const timeout = setTimeout(expire, this.timeoutMs);
      this.pending.set(id, { resolve, reject, timeout, expire });
      try {
        this.post({ ...data, type, id });
      } catch (error) {
        this.receive({ id, type: 'error', message: error.message });
      }
    });
  }
  receive(data) {
    ...
    if (data.type === 'progress') {
      request.timeout = setTimeout(request.expire, this.timeoutMs);
      return true;
    }
