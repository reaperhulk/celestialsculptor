...
export function parseReplay(text){
  ...
  if(![1,2].includes(value?.version)||!value.config||!Array.isArray(value.commands)||value.commands.length>2048||!Number.isInteger(value.end_tick)||value.end_tick<0||value.end_tick>307200)throw new Error('This experiment format is unsupported or exceeds the 600-year limit.');
  return value;
}
