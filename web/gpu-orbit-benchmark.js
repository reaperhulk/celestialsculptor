...
export async function runGpuOrbitBenchmark({counts=[65,1024,4096,8192],rounds=3,fallback=false,progress=()=>{}}={}){
 ...
 try{
  ...
  try{cpu.advance(512);reference=cpu.state();}finally{cpu.free();}
  gpu.reset(initial);for(let i=0;i<16;i++)candidate=await gpu.advance(32);
  const before=orbitBalances(initial),after=orbitBalances(candidate),cpuAfter=orbitBalances(reference);
  ...
  const trajectory={bodies:64,ticks:512,...orbitError(reference,candidate),moonPhaseErrorsRadians:phaseErrors,relativeEnergyDrift:(after.energy-before.energy)/Math.abs(before.energy),cpuRelativeEnergyDrift:(cpuAfter.energy-before.energy)/Math.abs(before.energy),momentumResidual:Math.hypot(after.px-before.px,after.py-before.py)/before.momentumScale};
  ...
  return {supported:true,adapter:gpu.gravity.info,browser:navigator.userAgent,cases,trajectory,batchInvariant,livePhysicsEnabled:false,scope:'Collisionless moving-orbit qualification. Four KDK substeps per tick; positions, velocities and acceleration stay on the GPU between steps. One state readback per batch. CPU uses production f64 SIMD/tree forces; GPU uses tiled direct f32. Collisions, migration, escapes, histories, rendering and cross-device replay are not implemented by this prototype.'};
 }finally{gpu.dispose();}
}
