...
export class Soundscape {
  ...
  setVolume(value) {
    ...
    if (this.master)
      this.master.gain.setTargetAtTime(0.12 * this.volume, this.context.currentTime, 0.04);
  }
  async toggle() {
    if (!this.context) {
      ...
      this.master.gain.value = 0.12 * this.volume;
      this.master.connect(this.context.destination);
      this.ambient = this.context.createGain();
      this.ambient.gain.value = 0.04;
    ...
    if (enabled) this.event('launch');
    return this.enabled;
  }
  note(frequency, start, duration = 0.65) {
    if (!this.enabled || this.context?.state !== 'running' || this.voices >= 12) return;
    const ctx = this.context,
      osc = ctx.createOscillator(),
      gain = ctx.createGain(),
      now = ctx.currentTime + start;
    osc.type = 'sine';
    ...
    const notes =
      {
        launch: [220, 330],
        placed: [220, 330],
        seed: [164.81, 246.94, 329.63],
        nudge: [293.66, 349.23],
        collision: [130.81, 196],
        graze: [349.23, 293.66],
        disruption: [98, 73.42, 65.41],
        escape: [523.25, 392, 261.63],
        absorb: [110, 82.41],
        complete: [261.63, 329.63, 392, 523.25],
      }[kind] || [];
    notes.forEach((frequency, i) =>
      this.note(frequency, i * 0.12, kind === 'complete' ? 1.5 : 0.7),
    );
  }
