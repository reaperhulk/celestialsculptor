...
const challengeGuide=new ChallengeGuide({send,getState:()=>state,trySetup:async replay=>{if(!canPlay(profile,replay.config.mission))throw new Error('Complete earlier challenges first.');const original=await send('original');notebookEntries=preserveOriginal(storage,notebookEntries,original.replay,original.snapshot);saveEpoch++;await send('import',{replay:JSON.stringify(replay)});renderer?.fit();await autosave();toast('Example setup ready at year zero. Your previous run is in the notebook. Change a condition and run.');}});
