...
export class GpuGravity {
  ...
  constructor(device, pipeline, info) {
    this.device = device;
    ...
    const d = this.device;
    this.input = d.createBuffer({
      size: this.capacity * 16,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC,
    });
    this.output = d.createBuffer({
    ...
    this.buffers = [this.input, this.output, this.readback, this.params];
    this.packed = new Float32Array(this.capacity * 4);
    this.bindings = d.createBindGroup({
      layout: this.pipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: { buffer: this.input } },
  ...
  async compute(particles) {
    ...
    if (!Number.isInteger(count) || count < 2 || count > 8192 || !particles.every(Number.isFinite))
      throw new Error('Use 2–8192 finite gravity particles');
    if (this.busy) throw new Error('A gravity dispatch is already running');
    this.busy = true;
    try {
      this.ensure(count);
      for (let i = 0; i < count; i++) {
        this.packed[i * 4] = particles[i * 3];
        this.packed[i * 4 + 1] = particles[i * 3 + 1];
        this.packed[i * 4 + 2] = particles[i * 3 + 2];
      }
      const params = new ArrayBuffer(16);
      new Uint32Array(params)[0] = count;
      new Float32Array(params).set([1e-8, 39.47841760435743, 0], 1);
      const d = this.device;
      d.queue.writeBuffer(this.input, 0, this.packed, 0, count * 4);
      ...
      pass.end();
      encoder.copyBufferToBuffer(this.output, 0, this.readback, 0, count * 8);
      d.queue.submit([encoder.finish()]);
      await this.readback.mapAsync(GPUMapMode.READ, 0, count * 8);
      const result = new Float32Array(this.readback.getMappedRange(0, count * 8).slice(0));
      this.readback.unmap();
      return result;
    } finally {
