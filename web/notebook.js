...
export const METRICS=[['years','Years'],['planets','Worlds'],['calm','Calm worlds'],['formed','Formed from debris'],['moons','Bound moons'],['habitable','Potential gardens'],['mass','Retained Earth masses'],['collisions','Mergers'],['ejections','Escapes']];
...
export function readNotebook(storage){
 ...
 return data.entries.filter(value=>{try{parseReplay(value.replay);return typeof value.id==='string'&&value.id.length<100&&typeof value.name==='string'&&value.name.length<=64&&value.summary&&METRICS.every(([key])=>Number.isFinite(value.summary[key]));}catch{return false;}});
 }catch{return [];}
}
